...
from mmdet.core import (anchor_inside_flags, build_anchor_generator,
                        ...
                        images_to_levels, multi_apply, multiclass_nms, unmap, bbox_overlaps)
...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.sampling = loss_cls['type'] not in [
            'FocalLoss', 'GHMC', 'QualityFocalLoss', 'IoUJointLoss'
        ]
        ...
        self.use_ijl = use_ijl
        if self.use_ijl:
            self.loss_cls = build_loss(loss_cls_ijl)
        else:
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        if self.use_ijl:
            ...
            if not self.reg_decoded_bbox:
                ...
                pos_bbox_pred_decoded = self.bbox_coder.decode(pos_anchors, pos_bbox_pred.detach())
                pos_bbox_targets_decoded = self.bbox_coder.decode(pos_anchors, pos_bbox_targets.detach())
                score[pos_inds] = bbox_overlaps(
                    pos_bbox_pred_decoded.detach(),
                    pos_bbox_targets_decoded.detach(),
                    is_aligned=True)
            else:
                score[pos_inds] = bbox_overlaps(
                    pos_bbox_pred.detach(),
                    pos_bbox_targets.detach(),
                    is_aligned=True)
